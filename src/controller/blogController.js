...
exports.createBlog = async (req, res) => {
  try {
    ...
    if (Object.keys(req.body).length == 0) return res.status(400).send({ status: false,
       message: "please provide key in request body" })
...
exports.deleted = async (req, res) => {
  try {
    ...
    let data = await blogModel.findOneAndUpdate({ _id : Id, isDeleted : false }, { isDeleted : true ,
      deletedAt :new Date(Date.now())}, { new: true })
